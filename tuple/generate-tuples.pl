...
open my $fh, ">", "tuples_gen.go" or die "failed to open: $!";
...
for my $n(2..16) {
    ...
    say $fh "}";
    say $fh "";
    say $fh "func (t Tuple${n}[$types]) String() string {";
    say $fh "	return fmt.Sprintf(\"(", join(", ", map { '%v' } 1..$n), ")\", ", join(", ", map { "t.V$_" } 1..$n), ")";
    say $fh "}";
}

system("gofmt -s -w tuples_gen.go");
