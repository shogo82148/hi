...
for my $n(2..16) {
    ...
    my $args = mapJoin { "o$_ Optional[T$_]" } $n;
    ...
    say $fh "func Zip${n}[$types any]($args) Optional[tuple.Tuple${n}[$types]] {";
    say $fh " if (" . (join " && ", map { "o$_.valid" } 1..$n) . ") {";
    say $fh "  return Optional[tuple.Tuple${n}[$types]]{valid: true, value: tuple.Tuple${n}[$types]{" . (mapJoin { "V$_: o$_.value" } $n) . "}}";
    say $fh " } else {";
    say $fh "  return Optional[tuple.Tuple${n}[$types]]{}";
    say $fh " }";
    say $fh "}";
}
